...
export async function boardScreen(path: string[], me: Me, query: URLSearchParams): Promise<Screen | null> {
    ...
    if (kind === 'projects') {
        ...
        const fields: Field[] = [field('title', 'Tittel', 'text', { required: true }), field('goal', 'Mål', 'multiline'), field('dueDate', 'Frist', 'date', { nullable: true }), select('linkedProjectId', 'Konsert / prosjekt', options(projectsData.projects, p => p.id, p => p.name), true)];
        if (!id) {
